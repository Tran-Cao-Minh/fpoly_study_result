...
if (window.matchMedia("(min-width: 1025px)").matches) {
    ...
    if (
        sessionStorage.getItem('key_word') != null && 
        sessionStorage.getItem('key_word') != ''
    ) {
        document.querySelector(".search-area>.fas").click();
    }
}
...
document.getElementById("to-top").addEventListener("click", function () {
    ...
    let full_screen_notification = document.getElementById("full-screen-notification");
    full_screen_notification.querySelector('.content').innerHTML = 
        'được tạo bởi trần cao minh';
    full_screen_notification.style.display = "flex";
});

document.getElementById("full-screen-notification").addEventListener("click", function () {
